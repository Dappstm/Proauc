...
# === OFFLINE HUMOR GENERATOR (No AI Needed) ===
FUNNY_REACTIONS = [
    "WHAT JUST HAPPENED", "TOTAL CHAOS", "LEGENDARY FAIL", "INSANE MOMENT",
    "BRUH MOMENT", "RIDICULOUS!", "UNREAL!", "OUTLAW ENERGY", "GONE WRONG",
    "UNSTOPPABLE MADNESS", "WHEN NPCS SNAP"
]

RDR2_CONTEXT = [
    "COWBOY FAILS", "WILD WEST CHAOS", "ARTHUR’S REVENGE",
    "DEADLY DUEL", "LAW GONE WILD", "HORSE GONE CRAZY", "SALOON DISASTER"
]


def generate_funny_labels(titles_and_thumbnails):
    """Generate short, funny labels locally without AI."""
    print("😂 Generating funny labels (offline)...")
    labels = []
    for item in titles_and_thumbnails:
        title = item["title"].lower()

        if "horse" in title:
            labels.append(random.choice(["HORSE MADNESS", "WHY HORSE WHY", "RIP HORSE"]))
        elif "sheriff" in title or "law" in title:
            labels.append(random.choice(["LAWLESS", "SHERIFF GONE WILD"]))
        elif "arthur" in title:
            labels.append(random.choice(["ARTHUR BE LIKE", "NOT AGAIN ARTHUR"]))
        elif "fight" in title or "duel" in title:
            labels.append(random.choice(["DEADLY DUEL", "SHOWDOWN TIME"]))
        else:
            # Mix in random funny + context
            funny = random.choice(FUNNY_REACTIONS)
            context = random.choice(RDR2_CONTEXT)
            labels.append(f"{funny}" if random.random() < 0.6 else context)

    return labels


def generate_main_title(titles_and_thumbnails):
    """Generate a bold clickbait YouTube title (offline)."""
    print("🏷️ Generating main title (offline)...")

    base_titles = [
        "TOP RDR2 MOMENTS THAT BROKE THE GAME",
        "FUNNIEST RED DEAD FAILS EVER",
        "WHEN NPCS GO CRAZY IN RDR2",
        "BEST OF WILD WEST CHAOS",
        "RIDICULOUS RDR2 MOMENTS COMPILATION",
        "COWBOY LOGIC BE LIKE",
        "ARTHUR MORGAN'S FUNNIEST MOMENTS"
    ]
...
def compose_short(clip_data, output_filename="final_short.mp4"):
    """
    Compose a YouTube short using rule-based funny labels and title.
    clip_data = list of dicts:
      [{"path": "video1.mp4", "title": "...", "thumbnail": "..."}, ...]
    """
    print("\n🎬 Building video from clips...")
    titles_and_thumbnails = [{"title": c["title"], "thumbnail": c.get("thumbnail", "")} for c in clip_data]

    # Generate offline funny labels + main title
